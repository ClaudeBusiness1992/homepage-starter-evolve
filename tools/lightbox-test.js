...
function pass(label, note = '') {
  passed++;
  const txt = `  \x1b[32m✓ PASS\x1b[0m  ${label}${note ? '  (' + note + ')' : ''}`;
  console.log(txt);
  results.push({ ok: true, label, note });
}
function fail(label, note = '') {
  failed++;
  const txt = `  \x1b[31m✗ FAIL\x1b[0m  ${label}${note ? '  — ' + note : ''}`;
  console.log(txt);
  results.push({ ok: false, label, note });
}
function info(label) {
  console.log(`  \x1b[33mℹ INFO\x1b[0m  ${label}`);
}
...
(async () => {
  ...
  try {
    ...
    if (res && res.ok()) {
      ...
      fail('Seite geladen', `HTTP ${res?.status() ?? '?'}`);
      await browser.close();
      process.exit(1);
    }
  } catch (e) {
  ...
  // Lightbox anfangs geschlossen?
  const initiallyOpen = await page.$eval('#lightbox', el => el.classList.contains('is-open')).catch(() => false);
  (!initiallyOpen)
  ...
  if (galWithLb > 0) {
    pass('.gallery-img Elemente haben data-lightbox', `${galWithLb} von ${galTotal}`);

    // URLs prüfen
    const srcs = await page.$$eval('#gallery .gallery-img[data-lightbox]', els =>
      els.map(el => el.dataset.lightbox)
    );
    const picsumCount = srcs.filter(s => s && s.includes('picsum.photos')).length;
    const otherCount  = srcs.length - picsumCount;

    if (picsumCount === srcs.length) {
      pass('data-lightbox enthält picsum.photos URLs', `Alle ${picsumCount} URLs`);
      info(`Beispiel-URL: ${srcs[0]}`);
    } else if (picsumCount > 0) {
      pass('data-lightbox enthält picsum.photos URLs (teilweise)', `${picsumCount}/${srcs.length}`);
      fail(`Nicht-picsum URLs gefunden`, `${otherCount} Items ohne picsum`);
    } else {
      fail('data-lightbox enthält picsum.photos URLs', `Keine picsum-URLs — Beispiel: ${srcs[0]}`);
    }
  } else if (galPlaceh > 0) {
    // Alle Items sind Platzhalter — kein src → kein data-lightbox (by design)
    pass('.gallery-img ohne data-lightbox = korrekt (alle sind Platzhalter)',
         `${galPlaceh} Platzhalter, kein src vorhanden`);
    info('Hinweis: data-lightbox wird nur gesetzt wenn item.src befüllt ist — Template-Verhalten korrekt');
  } else {
  ...
  // Wir brauchen ein Element mit data-lightbox. Falls keins existiert,
  // legen wir eines synthetisch an (um den JS-Pfad zu testen).
  let galLbTarget;
  let synthetic = false;
  ...
  if (galWithLb > 0) {
    ...
    await page.evaluate(() => {
      const el = document.querySelector('#gallery .gallery-img');
      if (el) {
    ...
    synthetic = true;
    galLbTarget = page.locator('#gallery .gallery-img[data-lightbox]').first();
    const injected = await galLbTarget.count();
    injected > 0
  ...
  if (await galLbTarget.count() > 0) {
    ...
    // Opacity VOR Hover
    const opBefore = await galLbTarget.evaluate(el =>
    ...
    // Opacity NACH Hover
    const opAfter = await galLbTarget.evaluate(el =>
      getComputedStyle(el, '::after').opacity
    );
    const imgFilter = await galLbTarget.locator('img').evaluate(img =>
      getComputedStyle(img).filter
    ).catch(() => 'n/a (kein img-Kind)');

    info(`::after opacity  vor Hover: ${opBefore}  |  nach Hover: ${opAfter}`);
    info(`img filter nach Hover: ${imgFilter}`);

    const opNum = parseFloat(opAfter);
    ...
    const hasFilter = imgFilter && imgFilter !== 'none' && (imgFilter.includes('grayscale') || imgFilter.includes('brightness'));
    if (hasFilter) {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SZENARIO 4  —  Erstes Gallery-Bild klicken → Lightbox öffnet
  // ══════════════════════════════════════════════════════════════════════════
  console.log('\n── Szenario 4: Gallery-Bild klicken → Lightbox öffnet ──');

  let expectedGalSrc = null;
  ...
  if (await galLbTarget.count() > 0) {
    ...
    const lbOpen = await page.locator('#lightbox.is-open').count() > 0;
    lbOpen
    ...
    // .lb-img.src gesetzt?
    const lbSrc = await page.locator('#lightbox .lb-img').getAttribute('src');
    info(`lb-img src: ${lbSrc}`);

    if (lbSrc && lbSrc !== '' && lbSrc !== 'about:blank') {
      pass('.lb-img src ist gesetzt nach Klick', lbSrc.substring(0, 70));
    } else {
      fail('.lb-img src ist gesetzt nach Klick', `src="${lbSrc}"`);
    }

    // Passt src zu data-lightbox?
    if (expectedGalSrc && lbSrc) {
      const match = lbSrc === expectedGalSrc
                 || lbSrc.includes(expectedGalSrc)
                 || expectedGalSrc.includes(lbSrc);
      match
        ? pass('.lb-img src stimmt mit data-lightbox überein')
        : fail('.lb-img src stimmt mit data-lightbox überein',
               `lb="${lbSrc}" vs data="${expectedGalSrc}"`);
    }
  } else {
  ...
  const openBeforeEsc = await page.locator('#lightbox.is-open').count() > 0;
  if (!openBeforeEsc) {
  ...
  const openAfterEsc = await page.locator('#lightbox.is-open').count() > 0;
  if (!openAfterEsc) {
  ...
  const aboutExists = await page.locator('#about').count() > 0;
  aboutExists
  ...
  const aphTotal  = await page.locator('#about .aphoto').count();
  const aphWithLb = await page.locator('#about .aphoto[data-lightbox]').count();
  const aphPlaceh = await page.locator('#about .aphoto-placeholder').count();
  ...
  if (aphWithLb > 0) {
    ...
    const aphSrcs = await page.$$eval('#about .aphoto[data-lightbox]', els =>
      els.map(el => el.dataset.lightbox)
    );
    const picsumAph = aphSrcs.filter(s => s && s.includes('picsum.photos')).length;
    ...
    info('Hinweis: data-lightbox wird nur bei m.photo !== "" gesetzt (Template-Design korrekt)');
  } else {
  ...
  if (aphWithLb > 0) {
    ...
    await page.evaluate(() => {
      const el = document.querySelector('#about .aphoto');
      if (el) {
    ...
    aphSynthetic = true;
    aphTarget = page.locator('#about .aphoto[data-lightbox]').first();
    const injected = await aphTarget.count();
    injected > 0
  ...
  if (await aphTarget.count() > 0) {
    ...
    const lbOpenAph = await page.locator('#lightbox.is-open').count() > 0;
    lbOpenAph
    ...
    if (lbOpenAph) {
      const aphLbSrc = await page.locator('#lightbox .lb-img').getAttribute('src');
      ...
      // .lb-bg klicken zum Schließen
      await page.locator('#lightbox .lb-bg').click({ force: true });
      await page.waitForTimeout(450);

      await shot(page, '07b-lightbox-closed-bg-click');

      const closedBg = await page.locator('#lightbox.is-open').count() === 0;
      closedBg
        ? pass('Lightbox schließt nach .lb-bg-Klick')
